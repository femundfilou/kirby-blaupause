...
async function createSvgSprite(inputPath, outputPath, projectAbbreviation) {
  try {
    ...
    const panelIcons = {};
    console.log(`\x1b[35mTransforming svgs to sprite\x1b[0m`);
    ...
    console.log("");
    console.log(`\x1b[1;32mAll done!\x1b[0m`);
...
async function createCmsPluginFile(panelIcons, projectAbbreviation) {
  ...
  console.log("");
  console.log(`\x1b[35mAdded icons to panel using plugin "${projectAbbreviation}-icons"\x1b[0m`);
}

function getOptimizationOptions(name) {
  // Customizable SVGO options can be added here
  return {
    plugins: [
      {
        name: 'preset-default',
